...
async fn place(
    ...
    let column = column.parse::<usize>().map_err(|_| (Status::BadRequest, "Bad Request".to_string()))?;
    if column == 0 {
        return Err((Status::BadRequest, "Bad Request".to_string()))
    }
    grid.place(column - 1, &team)
        .map_err(|err| {
...
fn random_board(state: &State<Day12>) -> Result<String, Status> {
    let mut rand = state.rand.write().map_err(|_| Status::InternalServerError)?;
